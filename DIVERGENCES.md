...
A character controller and a soft body never collide, in either direction. This is a rule
in the object layer pair filter rather than a filter at any one call site: characters
register their object layer as `JoltBodyClass::Character` and soft bodies as
`JoltBodyClass::SoftBody`, and `ObjectLayerPairFilterImpl` refuses that pair. It therefore
holds for the `CharacterVirtual` sweep, for the ground query Jolt's rigid `JPH::Character`
runs inside `PostSimulation` (which hardcodes its own body filter and offers no hook), and
for the contacts the virtual character's inner body would otherwise resolve against cloth
during the step. An earlier version of this used a `JPH::BodyFilter` on the sweep alone and
covered only the first of those three.

This is the same answer the engine gives: the **NvCloth** gem does not depend on PhysX at
all, so its cloth is never in the physics scene and a PhysX character cannot reach it.
NvCloth cloth collides only against an explicit list of primitives attached to named joints
(`IClothConfigurator`) - at most 32 spheres, with capsules built from pairs drawn from that
same 32-sphere budget. The skeleton drives the colliders, the colliders push the cloth, and
nothing pushes back.
...
anything colliding a shape against that face seeds GJK with a zero normal and Jolt asserts.
Measured on an unpinned crest with a character standing in it, the smallest face fell from
1.97e-4 to 8.1e-7 across five frames.
...
characters are excluded, and the exclusion is not configurable: there is no setting to let
