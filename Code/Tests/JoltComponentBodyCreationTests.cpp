...
namespace JoltPhysics
{
    ...
    TEST_F(JoltSoftBodyAttachmentTests, AnOffsetPutsTheClothWhereItIsWornRatherThanOnTheOrigin)
    {
        // A soft body is generated centred on its entity, which is wrong for anything worn:
        // a cape belongs at an actor's shoulders and an actor's origin is between its feet.
        // Moving the cloth to its own entity is not an option either - the gem that binds
        // cloth to a skeleton needs the cloth and the actor on one entity - so the offset
        // is how a cape gets to shoulder height at all.
        auto entity = AZStd::make_unique<AZ::Entity>("Sailor");
        entity->CreateComponent<AzFramework::TransformComponent>();
        auto* softBody = entity->CreateComponent<JoltSoftBodyComponent>();
        ...
        settings.m_size = AZ::Vector3(1.0f, 1.0f, 1.0f);
        settings.m_resolution = 4;
        // Behind and above: over the shoulders of a figure standing at the origin.
        settings.m_offset = AZ::Vector3(0.0f, -0.2f, 1.4f);

        entity->Init();
        AZ::TransformBus::Event(
            entity->GetId(), &AZ::TransformBus::Events::SetWorldTM,
            AZ::Transform::CreateTranslation(AZ::Vector3(0.0f, 0.0f, 3.0f)));
        entity->Activate();
        ...
        // Entity at z=3, cape offset 1.4 above it - so the cloth is up at 4.4, not at 3.
        EXPECT_TRUE(bounds.GetCenter().IsClose(AZ::Vector3(0.0f, -0.2f, 4.4f), 0.05f))
            << "the offset did not move the cloth off its entity origin";

        entity->Deactivate();
    }

    TEST_F(JoltSoftBodyAttachmentTests, AnOffsetTurnsWithTheEntityItIsWornOn)
    {
        // The offset is in the entity's own space, so a figure that turns carries what it
        // wears round with it. A world-space offset would leave the cape behind wherever
        // it was authored the moment the wearer faced another way.
        auto entity = AZStd::make_unique<AZ::Entity>("TurnedSailor");
        entity->CreateComponent<AzFramework::TransformComponent>();
        auto* softBody = entity->CreateComponent<JoltSoftBodyComponent>();
        ...
        settings.m_size = AZ::Vector3(1.0f, 1.0f, 1.0f);
        settings.m_resolution = 4;
        settings.m_offset = AZ::Vector3(1.0f, 0.0f, 0.0f);

        entity->Init();
        // Turned a quarter circle about z: the offset that pointed along +x now points +y.
        AZ::TransformBus::Event(
            entity->GetId(), &AZ::TransformBus::Events::SetWorldTM,
            AZ::Transform::CreateFromQuaternionAndTranslation(
                AZ::Quaternion::CreateRotationZ(AZ::Constants::HalfPi), AZ::Vector3(0.0f, 0.0f, 5.0f)));
        entity->Activate();
        ...
        const AZ::Vector3 centre = bounds.GetCenter();
        EXPECT_NEAR(centre.GetY(), 1.0f, 0.05f) << "the offset did not turn with the entity";
        EXPECT_NEAR(centre.GetX(), 0.0f, 0.05f);
